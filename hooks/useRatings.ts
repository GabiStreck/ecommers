...
const useRatings = () => {
    ...
    const [ratingStore, setRatingStore] = useLocalStorage<Ratings[]>(FILTER_RATING, []);
    ...
    async function getRatings() {
        try {
            ...
            if (!ratingStore?.length) {
                ...
                const ratingData = Array.from(ratingList).map(item => ({ name: `${item} star`, id: item, values: [item, item + 1] }))
